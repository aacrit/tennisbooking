...
- **Smart scheduling** — Scans every 5 minutes at 7 AM (when new slots drop), every 15 minutes otherwise, stops overnight
...
.github/workflows/
  scan.yml               Scheduled scraper (every 15 min)
```
...
| `NORMAL_INTERVAL_MINUTES` | 15 | Scan frequency 8:00 AM - midnight CT |
...
| 8:00 AM - Midnight | Every 15 min | Catch cancellations throughout the day |
...
- **GitHub Actions limits:** The workflow runs ~96 times/day (every 15 min, 18 hours). Each run uses ~2-3 minutes. Well within the free tier.
